...
export class RecoCardComponent implements OnInit {
  ...
  async createAskRec() {
    const isAskedReco = true;
    const recoID = await this.explorerService.createNewRecommendation(isAskedReco,
    ...
    if (recoID) {
      ...
      // const recoIds = await this.askedRecoService.getAskedRecoId();
      // console.log(recoIds);
